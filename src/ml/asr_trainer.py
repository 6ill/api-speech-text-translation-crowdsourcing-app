...
class ASRFineTuner:
    ...
    def _prepare_dataset(self, dataset: Dataset) -> Dataset:
        ...
        # Apply mapping
        return dataset.map(prepare_data, remove_columns=dataset.column_names, num_proc=1)
    ...
    def evaluate_comparative(
        ...
        def run_eval(model_instance):
            trainer = Seq2SeqTrainer(
                ...
                args=Seq2SeqTrainingArguments(output_dir="temp_eval", per_device_eval_batch_size=8, predict_with_generate=True, fp16=True),
        ...
        logger.info(f"Evaluation Result - Baseline WER: {baseline_wer}, New WER: {new_model_wer}")
    ...
    def load_static_dataset_from_s3(self, s3_key: str, local_extract_path: str = "temp_static_eval") -> Dataset:
        """
        Downloads the Zipped HF Dataset from S3, extracts it, and loads it.
        """
        from src.core.storage import StorageClient # Lazy import to avoid circular dependency
